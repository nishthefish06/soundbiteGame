...
// Kept separate from Room so the state machine itself never has to reason
// about "who is this being sent to."
export function redactSnapshotFor(snapshot, viewerId) {
  ...
  if (isActor || isRevealed) return snapshot;
  return { ...snapshot, currentPrompt: null };
}
